...
def wav_to_mcep_file(dataset:str, sr=SAMPLE_RATE, processed_filepath:str='dataset/processed_256/'):
    ...
    for speaker in speakers:
        ...
        allwavs_cnt = len(glob.glob(f'{dataset}/{speaker}/*.wav'))

        print(f'Total {allwavs_cnt} audio files!')

        os.makedirs(os.path.join(processed_filepath, speaker))

        values_of_one_speaker = list(d[speaker].values())
        ...
        for index, one_chunk in enumerate(chunks(values_of_one_speaker, CHUNK_SIZE)):
            ...
            newname = f'{speaker}_{index}'
            file_path_z = os.path.join(processed_filepath, speaker, newname)
            ...
            for start_idx in range(0, coded_sp.shape[1] - FRAMES + 1, FRAMES):
                ...
                if one_audio_seg.shape[1] == FRAMES:
                    ...
                    filePath = os.path.join(processed_filepath, speaker, temp_name)
...
if __name__ == "__main__":
    ...
    input_dir = 'dataset/vcc2020/speakers/'
    output_dir = 'dataset/processed_256/'
